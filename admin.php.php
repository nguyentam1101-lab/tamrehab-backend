...
function redirect(string $tab): never
{
...
// Keep order records linked to their product and ordered quantity.
...
try {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($action === 'save_product') {
            ...
            $db->beginTransaction();
            $productStatement = $db->prepare('SELECT id, product_type, stock_quantity FROM products WHERE id = ?');
            ...
            if ($id > 0) {
                ...
                if ($product['product_type'] === 'physical') {
                    ...
                    $stockStatement = $db->prepare('UPDATE products SET stock_quantity = stock_quantity - ? WHERE id = ? AND stock_quantity >= ?');
            ...
            $db->commit();
            $message = 'Đã thêm đơn hàng và cập nhật tồn kho nếu cần.';
            $tab = 'orders';
        } elseif ($action === 'delete_order' && $id > 0) {
            $statement = $db->prepare('DELETE FROM orders WHERE id = ?');
            $statement->execute([$id]);
