...
if (!file_exists($dbFile)) {
    $dbFile = dirname(__DIR__) . DIRECTORY_SEPARATOR . 'brain.db';
}
...
$stmt = $db->prepare('UPDATE orders SET status = :status, paid_at = CURRENT_TIMESTAMP, amount = COALESCE(NULLIF(:amount, 0), amount), content = COALESCE(NULLIF(:content, ""), content) WHERE order_id = :order_id AND status = "pending"');
$stmt->execute([
    ':status' => 'success',
    ':amount' => $amount,
