...
try {
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if ($action === 'save_product') {
            ...
            if ($id > 0) {
                $statement = $db->prepare('UPDATE customers SET name = ?, phone = ?, zalo = ?, registered_at = ? WHERE id = ?');
                $statement->execute([$name, $phone, $zalo, $registeredAt, $id]);
            } else {
                $statement = $db->prepare('INSERT INTO customers (name, phone, zalo, registered_at) VALUES (?, ?, ?, ?)');
                $statement->execute([$name, $phone, $zalo, $registeredAt]);
            }
...
<body>
    ...
    <?php elseif ($tab === 'customers'): ?>
        <div class="layout">
            <form method="post">
                <h2><?= $editCustomer ? 'Chỉnh sửa khách hàng' : 'Thêm khách hàng' ?></h2>
                <input type="hidden" name="action" value="save_customer"><input type="hidden" name="tab" value="customers"><input type="hidden" name="id" value="<?= h((string) ($editCustomer['id'] ?? 0)) ?>">
                <label>Tên</label><input name="name" value="<?= h($editCustomer['name'] ?? '') ?>" required>
                <label>Số điện thoại</label><input name="phone" value="<?= h($editCustomer['phone'] ?? '') ?>">
                <label>Zalo</label><input name="zalo" value="<?= h($editCustomer['zalo'] ?? '') ?>">
                <label>Ngày đăng ký</label><input type="date" name="registered_at" value="<?= h($editCustomer['registered_at'] ?? '') ?>">
                <div class="actions"><button class="primary"><?= $editCustomer ? 'Lưu thay đổi' : 'Thêm mới' ?></button><?php if ($editCustomer): ?><a href="?tab=customers">Hủy</a><?php endif; ?></div>
            </form>
            <table><tr><th>Tên</th><th>Điện thoại</th><th>Zalo</th><th>Ngày đăng ký</th><th></th></tr>
                <?php foreach ($customers as $customer): ?><tr><td><?= h($customer['name']) ?></td><td><?= h((string) ($customer['phone'] ?? '')) ?></td><td><?= h((string) ($customer['zalo'] ?? '')) ?></td><td><?= h((string) ($customer['registered_at'] ?? '')) ?></td><td><a href="?tab=customers&edit=<?= $customer['id'] ?>">Sửa</a> <form method="post"><input type="hidden" name="action" value="delete_customer"><input type="hidden" name="tab" value="customers"><input type="hidden" name="id" value="<?= h((string) $customer['id']) ?>"><button onclick="return confirm('Xóa khách hàng này?')">Xóa</button></form></td></tr><?php endforeach; ?>
            </table>
        </div>
    <?php else: ?>
        <div class="layout">
            <form method="post">
                <h2><?= $editOrder ? 'Chỉnh sửa đơn hàng' : 'Thêm đơn hàng' ?></h2>
                <input type="hidden" name="action" value="save_order"><input type="hidden" name="tab" value="orders"><input type="hidden" name="id" value="<?= h((string) ($editOrder['id'] ?? 0)) ?>">
                <label>Mã đơn hàng</label><input name="order_id" value="<?= h($editOrder['order_id'] ?? '') ?>" required>
                <label>Sản phẩm</label><select name="product_id">
                    <?php foreach ($products as $product): ?><option value="<?= $product['id'] ?>" <?= ($editOrder['product_id'] ?? '') == $product['id'] ? 'selected' : '' ?>><?= h($product['name']) ?></option><?php endforeach; ?>
                </select>
                <label>Số lượng</label><input type="number" name="quantity" value="<?= h((string) ($editOrder['quantity'] ?? 1)) ?>" min="1" required>
                <label>Số tiền</label><input type="number" name="amount" value="<?= h((string) ($editOrder['amount'] ?? '')) ?>" min="0" step="0.01" required>
                <label>Content</label><textarea name="content"><?= h($editOrder['content'] ?? '') ?></textarea>
                <label>Trạng thái</label><select name="status"><?php foreach (['pending', 'success'] as $status): ?><option value="<?= $status ?>" <?= ($editOrder['status'] ?? 'pending') === $status ? 'selected' : '' ?>><?= $status ?></option><?php endforeach; ?></select>
                <div class="actions"><button class="primary"><?= $editOrder ? 'Lưu thay đổi' : 'Thêm mới' ?></button><?php if ($editOrder): ?><a href="?tab=orders">Hủy</a><?php endif; ?></div>
            </form>
            <table><tr><th>Mã đơn</th><th>Sản phẩm</th><th>SL</th><th>Giá</th><th>Trạng thái</th><th></th></tr>
                <?php foreach ($orders as $order): ?><tr><td><?= h($order['order_id']) ?></td><td><?= h((string) ($order['product_name'] ?? '')) ?></td><td><?= h((string) ($order['quantity'] ?? 1)) ?></td><td><?= number_format((float) $order['amount'], 0, ',', '.') ?></td><td><?php if ($order['status'] === 'success'): ?><span style="background:#28a745;color:#fff;padding:4px 10px;border-radius:4px;font-weight:700;display:inline-block;">THANH TOÁN THÀNH CÔNG</span><?php else: ?><span style="background:#ffc107;color:#212529;padding:4px 10px;border-radius:4px;font-weight:700;display:inline-block;">Chờ thanh toán</span><?php endif; ?></td><td><a href="?tab=orders&edit=<?= $order['id'] ?>">Sửa</a> <form method="post"><input type="hidden" name="action" value="delete_order"><input type="hidden" name="tab" value="orders"><input type="hidden" name="id" value="<?= h((string) $order['id']) ?>"><button onclick="return confirm('Xóa đơn hàng này?')">Xóa</button></form></td></tr><?php endforeach; ?>
            </table>
        </div>
    <?php endif; ?>
</body>
